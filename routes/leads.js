...
router.post('/', leadsLimiter, async (req, res, next) => {
  try {
    ...
    // Can be temporarily disabled via DISABLE_RECAPTCHA=true env var
    // =========================================
    const recaptchaDisabled = process.env.DISABLE_RECAPTCHA === 'true';
    ...
    if (!recaptchaDisabled) {
      ...
      console.warn('[SPAM] reCAPTCHA disabled via DISABLE_RECAPTCHA env var — accepting submission from IP:', req.ip);
    }
